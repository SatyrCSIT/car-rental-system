            ...
            <nav class="flex items-center gap-1 sm:gap-2 text-sm">
                ...
                @auth
                    @if (auth()->user()->role === 'admin')
                        <a href="{{ route('admin.dashboard') }}" class="px-3 py-2 rounded-lg text-indigo-600 font-medium hover:bg-indigo-50 transition">Admin</a>
                    @endif
                    <a href="{{ route('rentals.index') }}" class="px-3 py-2 rounded-lg text-slate-600 hover:text-indigo-600 hover:bg-slate-100 transition">การจองของฉัน</a>
